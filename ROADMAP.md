...
This phase keeps the canonical package unchanged and supports selecting an OLE object in PowerPoint, saving or exporting `figure.mpl.png` on every platform, restoring the `Figure` in a separate Python console, and writing an updated graphic or object. Editable graphics use `.mpl.png`, `.mpl.pdf`, and `.mpl.svg` compound suffixes so they are visually distinct from ordinary files. Raw `.bin` and `.mplpkg` files remain implementation details rather than user-facing interchange formats.
